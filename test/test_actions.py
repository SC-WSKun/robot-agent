...
class TestWatchAction:
    ...
    def test_watch_action_metadata(self):
        """测试 WatchAction 元信息"""
        action = WatchAction()
        metadata = action.get_metadata()
        
        assert metadata.name == "watch"
        assert "vision" in metadata.capabilities
    
    def test_watch_action_initialization(self):
        """测试 WatchAction 初始化"""
        action = WatchAction()
        
        # 不提供 API key，应该使用 mock 模式
        action.initialize({})
    ...
    @pytest.mark.asyncio
    async def test_watch_action_execute_mock(self):
        """测试 WatchAction 执行（Mock 模式）"""
        action = WatchAction()
        action.initialize({})
        
        context = ActionContext(
            agent_state=AgentState.PATROLLING
        )
        ...
        assert "objects_detected" in result.output
        assert "emergency" in result.output
        assert isinstance(result.output["emergency"], bool)
    
    def test_watch_action_cleanup(self):
        """测试 WatchAction 清理"""
        action = WatchAction()
        action.initialize({})
        ...
        assert not action.is_initialized
        assert action.camera is None
...
class TestAlertAction:
    ...
    def test_alert_action_metadata(self):
        """测试 AlertAction 元信息"""
        action = AlertAction()
        metadata = action.get_metadata()
        
        assert metadata.name == "alert"
        assert "emergency_call" in metadata.capabilities
    
    def test_alert_action_initialization(self):
        """测试 AlertAction 初始化"""
        action = AlertAction()
        action.initialize({})
        
        assert action.is_initialized
        assert action.mcp_manager is not None
    
    @pytest.mark.asyncio
    async def test_alert_action_execute(self):
        """测试 AlertAction 执行"""
        action = AlertAction()
        action.initialize({})
        ...
        context = ActionContext(
            agent_state=AgentState.ALERT,
            input_data=event_details
        )
        
        result = await action.execute(context)
        
        assert result.success
        assert result.output["service_called"]
...
class TestRobotAgentActions:
    ...
    async def test_execute_action_chain(self):
        ...
        assert results[0].success  # watch action
        assert results[1].success  # speak action
